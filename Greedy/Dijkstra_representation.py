...
In the "Greedy Algorithms" lesson, we implemented the Dijkstra's Algorithm to find the distance of each node from the given source node. 
In this exercise, you'll implement the same Dijkstra's algorithm to find the length of the shortest path between a given pair of nodes, 
but this time we will have a better time complexity. First, let's build the graph.
...
def dijkstra(graph, start_node, end_node):
    ...
    distance_dict = {node: math.inf for node in graph.nodes} #math.inf to initiate infinity value 
