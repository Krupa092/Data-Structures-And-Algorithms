...
Write a program to find the longest possible sub sequence of consecutive(following contineously) numbers in the array. 
...
The Idea:
Every element of the given input_list could be a part of some subsequence. 
Therefore, we need a way (using a dictionary) to keep track if an element has already been visited. 
Also, store length of a subsequence if it is maximum. 
For this purpose, we have to check in forward direction, if the (element+1) is available in the given dictionary, in a "while" loop.
...
The steps would be:
Create a dictionary, such that the elements of input_list become the "key",
and the corresponding index become the "value" in the dictionary. 
We are creating a dictionary because the lookup time is considered to be constant in a dictionary.
...
Keep a track of length of longest subsequence visited so far. 
For the longest subsequence, store the smallest element (say start_element) and its index as well.
